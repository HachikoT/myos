...
// boot_main入口函数，加载elf格式的内核代码
void boot_main(void)
{
    // elf头存储在0x10000处
    struct elf32_header *elf_hdr = ((struct elf32_header *)0x10000);
    ...
    // 加载每一个程序段（忽略ph flags）
    struct elf32_phdr *ph, *end_ph;
    ph = (struct elf32_phdr *)((uintptr_t)elf_hdr + elf_hdr->e_phoff);
